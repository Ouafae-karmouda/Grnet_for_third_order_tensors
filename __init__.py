...
def t_product(A, B):
    ...
    A = np.fft.fft(A.cpu().detach().numpy())
    B = np.fft.fft(B.cpu().detach().numpy())
...
def t_product_tensors(A, B):
    n_1, n_2, n_3 = A.shape
    m_1, m_2, m_3 = B.shape
    assert n_2 == m_1 and n_3 == m_3, "Inner tensor dimensions must agree."
    A = torch.fft.fft(A).type(torch.DoubleTensor).cuda()
    B = torch.fft.fft(B).type(torch.DoubleTensor).cuda()
    C = torch.zeros(n_1, m_2, n_3).cuda()
    
    C[:, :, 0] = torch.matmul(A[:, :, 0], B[:, :, 0])
    half_n_3 = round(n_3 / 2)
    for i in range(1, half_n_3):
        C[:, :, i] = torch.matmul(A[:, :, i], B[:, :, i])
        C[:, :, n_3 - i] = torch.conj(C[:, :, i])
    if n_3 % 2 == 0:
        i = half_n_3
        C[:, :, i] = torch.matmul(A[:, :, i], B[:, :, i])
    C = torch.fft.ifft(C).real
    return C
...
def t_transpose(A):
    n_1, n_2, n_3 = A.shape
    A_t = torch.zeros((n_2, n_1, n_3))
    A_t[:, :, 0] = torch.conj(A[:, :, 0].T)
    for i in range(1, n_3):
        A_t[:, :, i] = torch.conj(A[:, :, n_3 - i].T)
    return A_t
